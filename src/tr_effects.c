...
void BeginShadowMap(ShadowMap shadowMap, Camera camera, Vector3 lightDirection, int cascade) {
    ...
    float tanHalfVFOV = tanf(DEG2RAD * (camera.fovy / 2.0f));
    // Horizontal half-angle follows from the vertical one: tan(h/2) = tan(v/2) * aspect.
    ...
    Matrix lightProj = MatrixOrtho(minX, maxX, minY, maxY, -maxZ - (maxZ - minZ) * 1.5, -minZ);
